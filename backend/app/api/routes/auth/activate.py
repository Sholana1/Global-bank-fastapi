...
async def activate_user(token:str, session: AsyncSession = Depends(get_session)):
    try:
        ...
        return {
            "message": "Account activated successfully", "email": user.email
        }
        ...
        if error_msg == "Activation token expired":
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "action": "Please that the link you click on is correct",
                }
            )
...
async def resend_activation_link(email_data: EmailRequestSchema, session:AsyncSession = Depends(get_session)):
    try:
        user = await user_auth_service.get_user_by_email(email_data.email, session, include_inactive=True)
        ...
        raise HTTPException(
                ...
                detail={
                    ...
                    "action": "Please try again later or contact support id the issue persist"
                }
            )